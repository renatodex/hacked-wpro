<?php
/**
get_option('wpro-aws-bucket')Plugin Name: WP Read-Only
					...
					<div class="wpro-service-div" id="wpro-service-s3-div" <?php if ($wproService == 'klandestino') echo ('style="display:none"'); ?> >
						...
						<table class="form-table">
							...
							<tr>
								...
								<td>
									<input name="wpro-aws-bucket" id="wpro-aws-bucket" type="text" value="<?php echo AWS_BUCKET; ?>" disabled="disabled" class="regular-text code" /><br />
									<span style="font-size:13px;font-weight:bold;margin-bottom:5px;display:block;">(bucket defined by environment vars)</span><br/>
...
//		$this->debug(print_r($data, true));
		...
		default:
			if (get_option('wpro-aws-virthost')) {
				$data['baseurl'] = 'http://' . trim(str_replace('//', '/', AWS_BUCKET . '/' . trim(get_option('wpro-folder'))), '/');
			} else {
				$data['baseurl'] = 'http://' . trim(str_replace('//', '/', AWS_BUCKET . '.s3.amazonaws.com/' . trim(get_option('wpro-folder'))), '/');
			}
		}
